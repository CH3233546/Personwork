...
class LoginScreen(Screen):
    ...
    def show_failure(self, req, res):
        if req.resp_status / 100 == 4:
            self.add_widget(Label(text='Invalid username or password. See readme to register.'))
...
class CloudCBScreen(Screen):
    ...
    def __init__(self, auth_token, **kwargs):
        ...
        layout.add_widget(Button(text='File Transmit',font_size=20,background_color='#00BFFF',on_press=self.webTransit))
        layout.add_widget(Label(text='Current text on clipboard',color='white',font_size=20,height=4,outline_color='blue',padding_x=15,padding_y=10))
        layout.add_widget(self.cloud_clip)
        layout.add_widget(Button(text='Refresh',font_size=20,background_color='#00BFFF' ,on_press=self.download))
        layout.add_widget(Label(text='Earlier text on your clipboard',font_size=20,outline_color='white'))
        layout.add_widget(TextInput(text=self.old_text))
        layout.add_widget(Button(text='Update Cloud Clipboard with this text',font_size=20,background_color='#00BFFF' , on_press=self.upload))
        self.add_widget(layout)

        self.download()
    def webTransit(self,*args):
        webbrowser.open("http://192.168.43.173:8000/")
